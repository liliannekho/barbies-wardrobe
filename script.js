...
// Event listener for career change
careerDropdown.addEventListener('change', () => {
    const selectedIndex = careerDropdown.value;
    barbie.career = new Career(
        careerDescriptions[selectedIndex].name,
        careerDescriptions[selectedIndex].description,
        careerIncomes[randomization(careerIncomes.length)],
        `${careerDescriptions[selectedIndex].name}-${careerIncomes[randomization(careerIncomes.length)]}`
    );
    barbie.render();
});
